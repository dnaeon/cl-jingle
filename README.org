...
#+begin_src lisp
  ...
  CL-USER> (setf (jingle:route *app* "/echo") #'echo-handler)
#+end_src
...
#+begin_src lisp
  ...
  (defun product-handler (params)
    "Handles requests for /api/v1/product/:name requests"
    (jingle:set-response-status :ok)
...
=JINGLE:WITH-JSON-RESPONSE=, but sets up the the response with a
=Content-Type: text/html; charset=utf-8= header and sends a short HTML
body in case of any errors.
